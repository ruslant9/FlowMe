...
export const EmojiPickerProvider = ({ children }) => {
    ...
    const variants = {
        hidden: isMobile 
            ? { y: "100%", transition: { duration: 0.2, ease: 'easeOut' } } 
            : { opacity: 0, scale: 0.9, y: 10, transition: { duration: 0.1, ease: 'easeOut' } },
        visible: isMobile 
            ? { y: 0, transition: { duration: 0.25, ease: 'easeOut' } } 
            : { opacity: 1, scale: 1, y: 0, transition: { duration: 0.15, ease: 'easeOut' } }
    };
