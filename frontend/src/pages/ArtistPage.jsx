...
const INITIAL_DISPLAY_LIMIT = 6; // --- ИСПРАВЛЕНИЕ: Константа для лимита отображения
const ArtistPage = () => {
...
// --- НАЧАЛО ИСПРАВЛЕНИЯ: Состояния для управления видимостью полного списка ---
            ...
            {albums.length > 0 && (
                 <div className="mb-12">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                        {/* --- ИСПРАВЛЕНИЕ: Используем ограниченный список --- */}
                        {displayedAlbums.map(album => (
                            <AlbumCard key={album._id} album={album} />
                        ))}
                    </div>
                    {/* --- ИСПРАВЛЕНИЕ: Кнопка "Показать еще" для альбомов --- */}
                    {albums.length > INITIAL_DISPLAY_LIMIT && !showAllAlbums && (
                        <div className="mt-6 text-center">
                            <button onClick={() => setShowAllAlbums(true)} className="px-6 py-2 bg-slate-200 dark:bg-white/10 text-sm font-semibold rounded-full hover:bg-slate-300 dark:hover:bg-white/20 transition-colors">
                                Показать еще
                            </button>
                        </div>
                    )}
            ...
            {singles && singles.length > 0 && (
                <div className="mb-12">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                     ...
                     {/* --- ИСПРАВЛЕНИЕ: Кнопка "Показать еще" для синглов --- */}
                    {singles.length > INITIAL_DISPLAY_LIMIT && !showAllSingles && (
                        <div className="mt-6 text-center">
                            <button onClick={() => setShowAllSingles(true)} className="px-6 py-2 bg-slate-200 dark:bg-white/10 text-sm font-semibold rounded-full hover:bg-slate-300 dark:hover:bg-white/20 transition-colors">
                                Показать еще
                            </button>
                        </div>
                    )}
