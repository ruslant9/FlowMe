// frontend/src/pages/MusicPage.jsx --- ПОЛНОСТЬЮ ИСПРАВЛЕННЫЙ ФАЙЛ ---
            ...
            {recommendations.popularArtists?.length > 0 && (
                 <section>
                    ...
                    <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-y-4 gap-x-2">
                         {recommendations.popularArtists.slice(0, 10).map(artist => <ArtistAvatar key={artist._id} artist={artist} size="xl" />)}
                    </div>
                </section>
            )}
        </div>
...
const MusicListView = ({ tracks, loading, onPlayTrack, onToggleSave, myMusicTrackIds, title, emptyMessage, onAddToPlaylist }) => {
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-1">
                {tracks.map((track, index) => (
                    <PlaylistTrackItem key={track._id} track={track} index={index + 1} onPlay={() => onPlayTrack(track)} isCurrent={track._id === currentTrack?._id} isPlaying={isPlaying} onToggleSave={onToggleSave} isSaved={myMusicTrackIds.has(track.sourceId || track._id)} onAddToPlaylist={onAddToPlaylist} />
                ))}
            </div>
        </div>
    );
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                ...
                <button onClick={onCreate} className="flex items-center space-x-2 text-sm bg-blue-500 hover:bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg transition-colors"><PlusCircle size={18} /><span>Создать</span></button>
            </div>
            {playlists.length === 0 ? (<div className="text-center p-20 text-slate-500">У вас еще нет плейлистов.</div>) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    {playlists.map(playlist => (
                        <PlaylistCard key={playlist._id} playlist={playlist} onClick={() => navigate(`/music/playlist/${playlist._id}`)} onDelete={onPlaylistDeleted} onEdit={onPlaylistUpdated} />
                    ))}
                </div>
            )}
        </div>
...
const SearchView = ({ query, setQuery, results, loading, onSearchMore, hasMore, onPlayTrack, loadingMore, onToggleSave, myMusicTrackIds, onAddToPlaylist }) => {
    ...
    return (
        <div className="space-y-8">
            <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} /><input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Что будем слушать?" className="w-full pl-12 pr-4 py-3 bg-white dark:bg-slate-800 rounded-lg text-lg focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
            ...
            {!query && !loading && (<div className="text-center p-20 text-slate-500"><Music size={48} className="mx-auto mb-4" /><p>Найдите любимые треки, альбомы, артистов и плейлисты.</p></div>)}
            {hasResults && (<div className="space-y-12">{artists?.length > 0 && (<section><SectionHeader title="Артисты" icon={MicVocal} /><div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-8 gap-4">{artists.map(artist => <ArtistAvatar key={artist._id} artist={artist} />)}</div></section>)}{albums?.length > 0 && (<section><SectionHeader title="Альбомы" icon={Disc} /><div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">{albums.map(album => <AlbumCard key={album._id} album={album} />)}</div></section>)}{playlists?.length > 0 && (<section><SectionHeader title="Плейлисты" icon={ListMusic} /><div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">{playlists.map(playlist => <PlaylistCard key={playlist._id} playlist={playlist} onClick={() => navigate(`/music/playlist/${playlist._id}`)} />)}</div></section>)}{tracks?.length > 0 && (<section><SectionHeader title="Треки" icon={Music} /><div className="space-y-1">{tracks.map((track, index) => (<PlaylistTrackItem key={track._id} track={track} index={index + 1} onPlay={() => onPlayTrack(track, tracks)} isCurrent={track._id === currentTrack?._id} isPlaying={isPlaying} onToggleSave={onToggleSave} isSaved={myMusicTrackIds.has(track.sourceId || track._id)} onAddToPlaylist={onAddToPlaylist} />))}</div>{hasMore && (<div className="mt-6 text-center"><button onClick={onSearchMore} disabled={loadingMore} className="px-6 py-2 bg-slate-200 dark:bg-white/10 text-sm font-semibold rounded-full hover:bg-slate-300 dark:hover:bg-white/20 flex items-center justify-center mx-auto disabled:opacity-70">{loadingMore ? (<><Loader2 className="animate-spin mr-2" size={16} /> <span>Загрузка...</span></>) : ('Загрузить еще')}</button></div>)}</section>)}</div>)}
        </div>
    );
};
...
const MusicPage = () => {
    ...
    // --- ПЕРЕРАБОТАННАЯ ЛОГИКА ЗАГРУЗКИ ДАННЫХ ---
    const fetchDataForTab = useCallback(async (tab) => {
        ...
        try {
            ...
            toast.error(`Не удалось загрузить раздел`);
        } finally {
            setIsLoading(false);
        }
    }, []);
    ...
    // Обработчик для обновления плейлистов
    ...
    // Обработчик для обновления "Моей музыки"
    const refetchMyMusic = useCallback(() => {
    ...
    useEffect(() => {
        window.addEventListener('refetchPlaylists', refetchPlaylists);
        window.addEventListener('myMusicUpdated', refetchMyMusic);
        return () => {
            window.removeEventListener('refetchPlaylists', refetchPlaylists);
            window.removeEventListener('myMusicUpdated', refetchMyMusic);
        };
    }, [refetchPlaylists, refetchMyMusic]);
        ...
        showConfirmation({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    window.dispatchEvent(new CustomEvent('refetchPlaylists'));
                } catch(e) {
    ...
    return (
        <PageWrapper>
            <CreatePlaylistModal 
                ...
                onPlaylistCreated={() => window.dispatchEvent(new CustomEvent('refetchPlaylists'))}
            />
            <EditPlaylistModal 
                isOpen={!!editingPlaylist}
                onClose={() => setEditingPlaylist(null)}
                playlist={editingPlaylist}
                onPlaylistUpdated={() => window.dispatchEvent(new CustomEvent('refetchPlaylists'))}
            />
            <UploadContentModal isOpen={isUploadModalOpen} onClose={() => setIsUploadModalOpen(false)} />
            <AddToPlaylistModal isOpen={isAddToPlaylistModalOpen} onClose={() => setAddToPlaylistModalOpen(false)} trackToAdd={trackToAdd} />

            <main className="flex-1 p-4 md:p-8">
                <div className="max-w-7xl mx-auto">
                    <div className="flex justify-between items-center mb-8"><h1 className="text-4xl font-bold">Музыка</h1></div>
                    <div className="hidden md:flex border-b border-slate-200 dark:border-slate-700/50 -mx-4 px-4 mb-8 overflow-x-auto no-scrollbar">
                        {navItems.map(item => (<TabButton key={item.key} active={activeTab === item.key} onClick={item.onClick} icon={item.icon}>{item.label}</TabButton>))}
                    </div>
                    <div className="md:hidden mb-6"><ResponsiveNav items={navItems} visibleCount={4} activeKey={activeTab} /></div>
                    <AnimatePresence mode="wait">
                        <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                            {activeTab === 'recommendations' && <RecommendationsView recommendations={recommendations} loading={isLoading} onPlayWave={handlePlayWave} />}
                            {activeTab === 'my-music' && <MusicListView tracks={myMusic} loading={isLoading} title="Моя музыка" emptyMessage="Вы еще не добавили ни одного трека." onPlayTrack={(track) => playTrack(track, myMusic)} onToggleSave={onToggleLike} myMusicTrackIds={myMusicTrackIds} onAddToPlaylist={handleAddToPlaylist} />}
                            {activeTab === 'history' && <MusicListView tracks={history} loading={isLoading} title="Недавно прослушанные" emptyMessage="Ваша история прослушиваний пуста." onPlayTrack={(track) => playTrack(track, history)} onToggleSave={onToggleLike} myMusicTrackIds={myMusicTrackIds} onAddToPlaylist={handleAddToPlaylist} />}
                            {activeTab === 'playlists' && <PlaylistsView playlists={playlists} loading={isLoading} onCreate={() => setIsCreatePlaylistModalOpen(true)} onPlaylistUpdated={setEditingPlaylist} onPlaylistDeleted={handlePlaylistDeleted} />}
                            {activeTab === 'search' && <SearchView query={searchQuery} setQuery={setSearchQuery} results={searchResults} loading={isLoading} onSearchMore={handleSearchMore} hasMore={hasMore} onPlayTrack={(track, tracklist) => playTrack(track, tracklist)} loadingMore={isLoadingMore} onToggleSave={onToggleLike} myMusicTrackIds={myMusicTrackIds} onAddToPlaylist={handleAddToPlaylist} />}
                        </motion.div>
                    </AnimatePresence>
                </div>
            </main>
