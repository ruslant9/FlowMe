...
const SinglePage = () => {
    ...
    const { dominantColor, textColor } = useDynamicAccent(track?.albumArtUrl);
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-center space-y-4 md:space-y-0 md:space-x-6 text-slate-900 dark:text-white">
                ...
                <div className="flex flex-col items-center md:items-start text-center md:text-left">
                    <span className="text-sm font-bold text-slate-600 dark:text-slate-300">Сольный трек (сингл)</span>
                    <h1 className="text-4xl md:text-6xl font-extrabold break-words mt-1">{track.title}</h1>
                    <div className="flex items-center space-x-2 mt-4 text-sm">
                        <Link to={`/artist/${primaryArtist._id}`}>
                            <Avatar size="sm" username={primaryArtist.name} avatarUrl={primaryArtist.avatarUrl} />
                        </Link>
                        <div className="text-slate-800 dark:text-white">{renderArtistLinks(track.artist)}</div>
                        <span className="opacity-70 text-slate-600 dark:text-slate-400">• {track.releaseDate ? format(new Date(track.releaseDate), 'LLLL yyyy', { locale: ru }) : ''} • 1 трек, {totalMinutes} мин.</span>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-8">
                <div className="flex items-center space-x-4 mb-8">
                    <button 
                        ...
                        className="px-8 py-3 bg-blue-600 text-white font-bold rounded-full flex items-center space-x-2 hover:scale-105 transition-transform"
                    >
