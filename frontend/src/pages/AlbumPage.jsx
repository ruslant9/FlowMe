...
const AlbumPage = () => {
    ...
    const { dominantColor, textColor } = useDynamicAccent(album?.coverArtUrl);
    ...
    return (
        <main className="flex-1 overflow-y-auto p-6 md:p-8">
            <button 
                onClick={() => navigate(-1)} 
                className="flex items-center space-x-2 text-sm z-10 bg-slate-200 dark:bg-slate-700/50 rounded-full px-3 py-1.5 shadow-lg hover:scale-105 hover:bg-slate-300 dark:hover:bg-slate-700 transition-all font-semibold mb-6"
            >
                <ArrowLeft size={16} strokeWidth={2.5} />
                <span>Назад</span>
            </button>

            <div className="flex flex-col md:flex-row items-center md:items-center space-y-4 md:space-y-0 md:space-x-6 text-slate-900 dark:text-white">
                ...
                <div className="flex flex-col items-center md:items-start text-center md:text-left">
                    ...
                    <h1 className="text-4xl md:text-6xl font-extrabold break-words mt-1">{album.title}</h1>
                    <div className="flex items-center space-x-2 mt-4 text-sm flex-wrap justify-center md:justify-start">
                        ...
                        <Link to={`/artist/${album.artist._id}`} className="font-bold hover:underline text-slate-800 dark:text-white">{album.artist.name}</Link>
                        <span className="opacity-70 text-slate-600 dark:text-slate-400">• {album.releaseDate ? format(new Date(album.releaseDate), 'LLLL yyyy', { locale: ru }) : ''} • {album.tracks.length} треков, {totalMinutes} мин.</span>
                        {album.totalPlayCount > 0 && (
                            <span className="opacity-70 flex items-center space-x-1 text-slate-600 dark:text-slate-400">
                                <span>•</span>
            ...
            <div className="mt-8">
                <div className="flex items-center space-x-4 mb-8">
                    <button 
                        ...
                        className="px-8 py-3 font-bold rounded-full flex items-center space-x-2 hover:scale-105 transition-transform bg-blue-600 text-white"
                    >
