// frontend/src/components/modals/EditPlaylistModal.jsx
...
const EditPlaylistModal = ({ isOpen, onClose, playlist, onPlaylistUpdated }) => {
    ...
    useEffect(() => {
        if (isOpen && playlist) {
            ...
            setVisibility(visibilityOptions.find(opt => opt.id === playlist.visibility) || visibilityOptions[0]);
        }
    }, [isOpen, playlist]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await axios.put(`${API_URL}/api/playlists/${playlist._id}`, { 
                name, 
                description, 
            ...
            toast.success(`Плейлист обновлен!`, { id: toastId });
            onPlaylistUpdated(res.data);
            onClose();
        } catch (error) {
            toast.error(error.response?.data?.message || 'Ошибка обновления.', { id: toastId });
        } finally {
            setLoading(false);
        }
    };
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}
                    ...
                    <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
                        ...
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-xl font-bold">Редактировать плейлист</h2>
                                            ...
                                            <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white dark:bg-slate-700 py-1 text-base shadow-lg ring-1 ring-black/5 z-10">
                                                {visibilityOptions.map((option) => (
                                                    <Listbox.Option key={option.id} className={({ active }) => `relative cursor-pointer select-none py-2 pl-10 pr-4 ${ active ? 'bg-blue-100 dark:bg-blue-600' : '' }`} value={option}>
                                                        {({ selected }) => (
                                                            <><span className={`block truncate ${ selected ? 'font-medium' : 'font-normal' }`}>{option.name}</span>{selected ? <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-blue-600 dark:text-white"><Check className="h-5 w-5"/></span> : null}</>
                                                        )}
                                                    </Listbox.Option>
                                                ))}
                                            </Listbox.Options>
