...
const UserListModal = ({ isOpen, onClose, user, listType, initialTitle, items: initialItems, postOwnerId }) => {
    ...
    const fetchData = useCallback(async () => {
        if (!isOpen || !endpoints || listType === 'user') return;
    ...
    }, [isOpen, endpoints, isOwnProfile, listType]);
        ...
        <AnimatePresence>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={handleClose}
                ...
                <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
                    ...
                    {endpoints && !isOwnProfile && endpoints?.mutual && (
                    ...
                    <div className="flex-1 overflow-y-auto -mr-2 pr-2 space-y-2">
                        ...
                        : (filteredItems.map(item => {
                            ...
                            // --- НАЧАЛО ИСПРАВЛЕНИЯ ---
                            const border = item.premiumCustomization?.avatarBorder;
                            ...
                            <Link to={linkTo} key={item._id} onClick={handleClose} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                                ...
                                <div className={`relative rounded-full ${borderClass}`} style={staticBorderStyle}>
                                    <Avatar
                                        ...
                                        size="md"
                                        isPremium={item.premium?.isActive}
                                        customBorder={border}
                                    />
                                </div>
                                {/* --- КОНЕЦ ИСПРАВЛЕНИЯ --- */}
