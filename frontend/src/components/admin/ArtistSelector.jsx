// frontend/components/admin/ArtistSelector.jsx
...
const ArtistSelector = ({ artists, value, onChange, required = true }) => {
    ...
    const selectedArtist = useMemo(() => artists.find(a => a._id === value) || null, [artists, value]);
    const handleSelectAndClose = (artist) => {
        onChange(artist ? artist._id : '');
        setIsModalOpen(false);
    };
    const filteredArtists = query === '' ? artists : artists.filter((artist) => artist.name.toLowerCase().includes(query.toLowerCase()));
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsModalOpen(false)}
                    ...
                    <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={(e) => e.stopPropagation()}
                        ...
                        <div className="relative mb-4">
                            ...
                            <input type="text" placeholder="Поиск по имени..." value={query} onChange={(e) => setQuery(e.target.value)}
                                className="w-full pl-10 pr-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" autoFocus />
                        </div>
                        <div className="flex-1 overflow-y-auto -mr-2 pr-2 space-y-2">
                            {filteredArtists.map(artist => (
                                <div key={artist._id} onClick={() => handleSelectAndClose(artist)} className="flex items-center space-x-4 p-2 rounded-lg cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800">
                                    <Avatar size="md" username={artist.name} avatarUrl={artist.avatarUrl} />
                                    <div className="flex-1 min-w-0"><p className="font-semibold truncate">{artist.name}</p></div>
                                </div>
                            ))}
                        </div>
                    </motion.div>
