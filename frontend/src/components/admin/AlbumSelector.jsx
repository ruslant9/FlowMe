                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}
                    ...
                    <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={(e) => e.stopPropagation()}
                        ...
                        <div className="relative mb-4">
                            ...
                            <input type="text" placeholder="Поиск по названию или артисту..." value={query} onChange={(e) => setQuery(e.target.value)}
                        ...
                        <div className="flex-1 overflow-y-auto -mr-2 pr-2 space-y-2">
                            <div onClick={() => onSelect(null)} className="flex items-center space-x-4 p-2 rounded-lg cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800">
                                <div className="w-12 h-12 flex-shrink-0 bg-slate-200 dark:bg-slate-700 rounded-md flex items-center justify-center"><Music/></div>
                                <div className="flex-1 min-w-0"><p className="font-semibold">-- Сольный трек (сингл) --</p></div>
                            </div>
...
const AlbumSelector = ({ albums, value, onChange, disabled = false }) => {
    ...
    return (
        <>
            ...
            <AlbumSelectorModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} albums={albums} onSelect={handleSelect} />
        </>
    );
};
