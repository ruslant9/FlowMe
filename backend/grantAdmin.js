...
const grantAdminRole = async () => {
  ...
  console.log('\x1b[36m%s\x1b[0m', '--- Запуск скрипта для выдачи прав администратора ---');
  ...
  try {
    ...
    if (user.role === 'admin') {
        console.log('\x1b[33m%s\x1b[0m', '   Этот пользователь уже является администратором. Обновление не требуется.');
    } else {
        console.log(`3. Обновление роли пользователя на "admin"...`);
        user.role = 'admin';
        await user.save();
        console.log('\x1b[32m%s\x1b[0m', '   Роль успешно обновлена.');
    }
    
    console.log('\n\x1b[32m%s\x1b[0m', `--- УСПЕХ! Пользователь ${user.username} теперь имеет права администратора. ---`);
